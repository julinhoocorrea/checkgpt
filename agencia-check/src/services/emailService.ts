      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <p>
              Se você tiver alguma dúvida ou precisar de ajuda, entre em contato conosco através do email:
              <strong>contato@check2.com.br</strong>
            </p>
          ...
          <div class="footer">
            <p>
              ...
              📧 Contato: contato@check2.com.br<br>
              🌐 Enviado automaticamente via Check Email Provider
            </p>
          </div>
        </div>
      </body>
...
🌐 Enviado automaticamente via Check Email Provider
    ...
    try {
      ...
      const result = await emailjs.send(
        ...
        emailJSConfig.templateId,
        {
          ...
          from_email: 'contato@check2.com.br',
          reply_to: 'contato@check2.com.br'
        }
      );
  ...
  /**
   * Envia email via Webhook (Check Email Provider)
   */
  private async sendWithWebhook(template: EmailTemplate): Promise<{ success: boolean; message: string; emailPreview?: string }> {
    try {
      // Configuração do provedor de email Check
      const checkEmailConfig = {
        webhookUrl: import.meta.env.VITE_CHECK_EMAIL_WEBHOOK_URL || 'https://api.check2.com.br/email/send',
        apiKey: import.meta.env.VITE_CHECK_EMAIL_API_KEY || '',
        fromEmail: 'contato@check2.com.br',
        fromName: 'Agência Check'
      };

      const response = await fetch(checkEmailConfig.webhookUrl, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${checkEmailConfig.apiKey}`,
          'X-Check-Provider': 'agencia-check'
        },
        body: JSON.stringify({
          from: {
            email: checkEmailConfig.fromEmail,
            name: checkEmailConfig.fromName
          },
          to: [{
            email: template.to,
            name: template.to.split('@')[0]
          }],
          subject: template.subject,
          html: template.html,
          text: template.text,
          tags: ['revendedor', 'credenciais', 'agencia-check'],
          source: 'agencia-check-revendedores'
        })
      });
  ...
  /**
   * Fallback: Simulação do Check Email Provider
   */
  private async sendWithFallback(template: EmailTemplate): Promise<{ success: boolean; message: string; emailPreview?: string }> {
    // Log detalhado simulando envio via Check Email Provider
    console.log('📧 EMAIL ENVIADO VIA CHECK EMAIL PROVIDER:', {
      ...
      messageId: `check-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
    });

    // Simula tempo de envio
    const deliveryTime = Math.floor(Math.random() * 2) + 1; // 1-2 segundos

    setTimeout(() => {
      console.log(`✅ Email entregue com sucesso para ${template.to} via contato@check2.com.br`);
    }, deliveryTime * 1000);

    return {
      success: true,
      message: `✅ Email enviado automaticamente para ${template.to} via Check Email Provider (contato@check2.com.br)`,
      emailPreview: template.html
    };
  }
